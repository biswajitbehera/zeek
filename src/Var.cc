...
static void make_var(ID* id, IntrusivePtr<BroType> t, init_class c,
                     IntrusivePtr<zeek::detail::Expr> init, attr_list* attr, zeek::detail::DeclType dt,
                     bool do_init)
	{
	if ( id->Type() )
		{
		if ( id->IsRedefinable() || (! init && attr && ! IsFunc(id->Type()->Tag())) )
			...
			if ( dt != zeek::detail::DeclType::VAR_REDEF )
				id->Warn("redefinition requires \"redef\"", redef_obj, true);
			}

		else if ( dt != zeek::detail::DeclType::VAR_REDEF || init || ! attr )
			{
			if ( IsFunc(id->Type()->Tag()) )
	...
	if ( do_init )
		{
		if ( c == INIT_NONE && dt == zeek::detail::DeclType::VAR_REDEF && t->IsTable() &&
	...
	if ( dt == zeek::detail::DeclType::VAR_CONST )
		{
	...
	if ( dt == zeek::detail::DeclType::VAR_OPTION )
		{
		if ( ! init )
...
public:
	...
	TraversalCode PreExpr(const zeek::detail::Expr* expr) override
		{
	...
	TraversalCode PostExpr(const zeek::detail::Expr* expr) override
		{
		if ( expr->Tag() == zeek::detail::EXPR_LAMBDA )
			scopes.pop_back();
...
namespace zeek::detail {
	...
	void add_global(ID* id, IntrusivePtr<BroType> t, init_class c,
		IntrusivePtr<zeek::detail::Expr> init, attr_list* attr, DeclType dt)
		{
	...
	IntrusivePtr<zeek::detail::Stmt> add_local(IntrusivePtr<ID> id, IntrusivePtr<BroType> t,
		...
		if ( init )
			...
			auto name_expr = make_intrusive<zeek::detail::NameExpr>(id, dt == zeek::detail::DeclType::VAR_CONST);
	...
	IntrusivePtr<zeek::detail::Expr> add_and_assign_local(IntrusivePtr<ID> id,
		IntrusivePtr<zeek::detail::Expr> init,
		IntrusivePtr<Val> val)
		{
		make_var(id.get(), nullptr, INIT_FULL, init, nullptr, zeek::detail::DeclType::VAR_REGULAR, false);
...
void add_global(ID* id, IntrusivePtr<BroType> t, init_class c,
	...
	{
	zeek::detail::add_global(id, t, c, init, attr, static_cast<zeek::detail::DeclType>(dt));
	}

IntrusivePtr<zeek::detail::Stmt> add_local(IntrusivePtr<ID> id,
                                    IntrusivePtr<BroType> t, init_class c,
                                    IntrusivePtr<zeek::detail::Expr> init, attr_list* attr,
                                    decl_type dt)
	{
	return zeek::detail::add_local(id, t, c, init, attr, static_cast<zeek::detail::DeclType>(dt));
	}
...
IntrusivePtr<zeek::detail::Expr> add_and_assign_local(IntrusivePtr<ID> id,
                                               ...
                                               IntrusivePtr<Val> val)
	{
	return zeek::detail::add_and_assign_local(id, init, val);
	}

void add_type(ID* id, IntrusivePtr<BroType> t, attr_list* attr)
	{
	zeek::detail::add_type(id, t, attr);
	}
...
void begin_func(ID* id, const char* module_name, function_flavor flavor,
                       ...
                       attr_list* attrs)
	{
	zeek::detail::begin_func(id, module_name, flavor, is_redef, t, attrs);
	}

void end_func(IntrusivePtr<zeek::detail::Stmt> body)
	{
	zeek::detail::end_func(body);
	}

// Gather all IDs referenced inside a body that aren't part of a given scope.
id_list gather_outer_ids(Scope* scope, zeek::detail::Stmt* body)
	{
	return zeek::detail::gather_outer_ids(scope, body);
	}

Val* internal_val(const char* name)
	{
	return zeek::internal_val(name);
	}

// Throws an internal error if not const
Val* internal_const_val(const char* name)
	{
	return zeek::internal_const_val(name);
	}

// Returns nullptr if not defined
Val* opt_internal_val(const char* name)
	{
	return zeek::opt_internal_val(name);
	}

double opt_internal_double(const char* name)
	{
	return zeek::opt_internal_double(name);
	}

bro_int_t opt_internal_int(const char* name)
	{
	return zeek::opt_internal_int(name);
	}

bro_uint_t opt_internal_unsigned(const char* name)
	{
	return zeek::opt_internal_unsigned(name);
	}

StringVal* opt_internal_string(const char* name)
	{
	return zeek::opt_internal_string(name);
	}

// Returns nullptr if not defined
TableVal* opt_internal_table(const char* name)
	{
	return zeek::opt_internal_table(name);
	}

ListVal* internal_list_val(const char* name)
	{
	return zeek::internal_list_val(name);
	}

BroType* internal_type(const char* name)
	{
	return zeek::internal_type(name);
	}

Func* internal_func(const char* name)
	{
	return zeek::internal_func(name);
	}

EventHandlerPtr internal_handler(const char* name)
	{
	return zeek::internal_handler(name);
	}
